function alliance() {
    this.init = function () {
        ...
        if (checkbox == false) {
            for (i = 0; i < titles.length; i++) {
                html += '<p onclick="new alliance().item(\'' + titles[i] + '\')">' + titles[i] +
                    '</p>'
            }
            html += '<p onclick="new alliance().general()">General</p>'
        }
        html += '<p onclick="new alliance().avgInit()">Season Averages</p>'
    ...
    this.item = function (title) {
        ...
        if (scouting !== null && scouting.titles.indexOf(title) > -1) {
            ...
            for (i = 0; i < arr.length; i++) {
                ...
                td.setAttribute("onclick", "delClick('" + team + "')")
            }
        } else {
    ...
    this.avgInit = function () {
        ...
        for (i = 0; i < choices.length; i++) {
            html += '<p onclick="new alliance().avgItem(\'' + choices[i] + '\')">' + choices[i] +
                '</p>'
        }
        html += '<p onclick="new alliance().avgGeneral()">General</p>'
        html += '<p onclick="menu()">Menu</p>'
        side.innerHTML += html
        document.getElementById("extras").appendChild(side)
    }
    ...
    this.avgItem = function (title) {
        var general = new alliance().avgGeneral()
        if (document.getElementById("divTable") !== null) {
            document.getElementById("extras").removeChild(document.getElementById("divTable"))
        }
        if (document.getElementById("divTable") !== null) {
